...
namespace CS3Comparation
{
    class GetComparationClient
    {
        static void Main(string[] args)
        {
            ...
            ListaScripts.Add("Tables", Results);
            ListaScripts.Add("TablesCreate", CreateStrucs(Results, "TablesCreate", ORIGEN, DESTINO, Root));

            //CREANDO ATRIBUTOS
            Resultados = ComparacionItems(ORIGEN, DESTINO, "Atributtes", Root);
            Results = Resultados.ToList();
            ListaScripts.Add("Atributtes", Results);
            ListaScripts.Add("AtributtesCreate",CreateStrucs(Results, "AtributtesCreate", ORIGEN, DESTINO, Root));
            
            //COMPARANDO TAMAÑO DE ATRIBUTOS
            Resultados = ComparacionItems(ORIGEN, DESTINO, "AtributtesLength", Root);
            Results = Resultados.ToList();
            ListaScripts.Add("AtributtesLength", Results);
            ListaScripts.Add("AtributtesLengthCreate", CreateStrucs(Results, "AtributtesLengthCreate", ORIGEN, DESTINO, Root));

            //CREANDO FORANEAS
            Resultados = ComparacionItems(ORIGEN, DESTINO, "Foreign", Root);
            Results = Resultados.ToList();
            ListaScripts.Add("Foreign", Results);
            ListaScripts.Add("ForeignCreate",CreateStrucs(Results, "ForeignCreate", ORIGEN, DESTINO, Root));

            //CREANDO INDICES
            Resultados = ComparacionItems(ORIGEN, DESTINO, "Index", Root);
            Results = Resultados.ToList();
            ListaScripts.Add("Index", Results);
            ListaScripts.Add("IndexCreate",CreateStrucs(Results, "IndexCreate", ORIGEN, DESTINO, Root));
        ...
        private static string QueryType(string Root,string BD,string type,string Objeto,string table)
        {
            string line;
            string query="";
            ...
            else if (type.Equals("AtributtesLengthCreate"))
            {
                System.IO.StreamReader file = new System.IO.StreamReader(@Root);
                while ((line = file.ReadLine()) != null)
                {
                ...
                query = query.Replace("<<OBJETO>>", Objeto);
                query = query.Replace("<<ORIGEN>>", BD);
                query = query.Replace("<<TABLA>>", table);
            }
        ...
        private static List<string> CreateStrucs(List<string> strucs,string type,string ORIGEN,string DESTINO,string Root)
        {
            ...
            switch (type)
            {
                ...
                case "AtributtesLengthCreate":
                     foreach (string a in strucs)
                    {
                        ...
                        {
                            TABLA = m.ToString();
                            string prueba = a.Replace(TABLA+"+", "");
                            Match m2 = regex.Match(prueba);
                            if (m2.Success)
                            {
                                OBJETO = m2.ToString();
                            }
                        ...
                        string ConnectionString = GetConecciont(ORIGEN);
                        string QueryString = QueryType(RootAux = GetQueryPath(Root, "AtributtesLengthCreate"), ORIGEN, "AtributtesLengthCreate", OBJETO, TABLA);
                        resultados = GetQueryConnection(QueryString,ConnectionString);
                    }
                    break;
                case "ForeignCreate":
        ...
        private static string GetQueryPath(string Root,string type)
        {
            ...
            {
                a = Root.Replace(Root = "\\Debug", "\\Debug\\SqlAtributtesLengthCreate.sql");
            }
            else if (type.Equals("ForeignCreate"))
            {
